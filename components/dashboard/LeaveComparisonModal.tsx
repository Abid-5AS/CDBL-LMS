    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            This request was modified and resubmitted. Compare the previous version with the current
            version.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4 mt-4">
            ...
            {versions.length > 1 && (
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium">Compare with version:</span>
                <select
                ...
                <Card>
                  <CardContent className="p-4 space-y-3">
                    <div>
                      ...
                      <p className="text-sm">{leaveTypeLabel[oldVersion.data.type] || oldVersion.data.type}</p>
                    </div>
                    <div>
                      <span className="text-xs font-medium text-muted-foreground">Dates:</span>
                      <p className="text-sm">
                        {formatDate(oldVersion.data.startDate)} → {formatDate(oldVersion.data.endDate)}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-sm whitespace-pre-wrap">{oldVersion.data.reason}</p>
                    </div>
                    {oldVersion.data.needsCertificate && (
                      <div>
                        <span className="text-xs font-medium text-muted-foreground">Certificate:</span>
                        <p className="text-sm">
                          {oldVersion.data.certificateUrl ? "Attached" : "Required"}
                        </p>
                      </div>
                    )}
                  </CardContent>
                  ...
                  <CardContent className="p-4 space-y-3">
                    ...
                    {currentLeave.needsCertificate && (
                      <div>
                        <span className="text-xs font-medium text-muted-foreground">Certificate:</span>
                        <p className="text-sm">
                          {currentLeave.certificateUrl ? "Attached" : "Required"}
                        </p>
                      </div>
                    )}
                  </CardContent>
