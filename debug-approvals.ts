...
async function debugApprovals() {
  ...
  console.log(`📋 Found ${allLeaves.length} SUBMITTED/PENDING leave requests:\n`);

  for (const leave of allLeaves) {
    console.log(`\n--- Leave Request #${leave.id} ---`);
    console.log(`Type: ${leave.type}`);
    console.log(`Status: ${leave.status}`);
    console.log(`Requester: ${leave.requester.name} (ID: ${leave.requester.id}, Role: ${leave.requester.role})`);
    ...
    if (leave.approvals.length === 0) {
      ...
      leave.approvals.forEach(app => {
        console.log(`  - Step ${app.step}: ${app.decision} by ${app.approver?.name || 'Unknown'} (ID: ${app.approverId}, Role: ${app.approver?.role})`);
  ...
  console.log(`✅ Query returned ${filteredLeaves.length} leave requests for HR_ADMIN\n`);

  if (filteredLeaves.length > 0) {
    filteredLeaves.forEach(leave => {
      console.log(`  - Leave #${leave.id}: ${leave.type}, Status: ${leave.status}, Requester: ${leave.requester.name}`);
    });
  } else {
    ...
    for (const leave of allLeaves) {
      ...
      const hasActionedApproval = leave.approvals.some(
        app => app.approverId === hrAdmin.id && ["FORWARDED", "APPROVED", "REJECTED"].includes(app.decision)
      );
      if (hasActionedApproval) {
        console.log(`    ❌ FILTERED OUT: HR_ADMIN already has FORWARDED/APPROVED/REJECTED decision`);
      } else {
        console.log(`    ✅ Should pass filter (no FORWARDED/APPROVED/REJECTED by HR_ADMIN)`);
        const pendingApproval = leave.approvals.find(app => app.approverId === hrAdmin.id && app.decision === "PENDING");
        if (pendingApproval) {
