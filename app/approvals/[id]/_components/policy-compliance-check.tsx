...
export function PolicyComplianceCheck({
  ...
  if (leaveType === "MEDICAL" && workingDays > 7) {
    checks.push({
      ...
      details:
        "Medical leave exceeding 7 days requires certificate (Policy 6.21.b)",
    });
  }
  ...
  if (leaveType === "MEDICAL" && workingDays > 7) {
    checks.push({
      ...
      details:
        "Employee must submit fitness certificate before resuming duty (Policy 6.21.c)",
    });
  }
  ...
  if (isDecemberYearEnd) {
    checks.push({
      ...
      details:
        "Leave request includes December 25-31 (year-end processing period)",
    });
  }
