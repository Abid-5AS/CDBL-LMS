...
/**
 * GET /api/admin/balance/import?template=true
 * Download CSV template for balance import
 */
export async function GET(req: NextRequest) {
  ...
  const template = ImportExportService.generateBalanceTemplate();

  return new NextResponse(template, {
    headers: {
      "Content-Type": "text/csv",
      "Content-Disposition": 'attachment; filename="balance_import_template.csv"',
    },
  });
}

/**
 * POST /api/admin/balance/import
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await ImportExportService.importBalances(csvContent, dryRun);
    ...
    console.error("POST /api/admin/balance/import error:", err);
    return NextResponse.json(
      error("internal_error", "Failed to import balances", traceId),
      { status: 500 }
    );
  }
}
