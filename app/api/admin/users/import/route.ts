...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await ImportExportService.importEmployees(csvContent, dryRun);
    ...
    console.error("POST /api/admin/users/import error:", err);
    return NextResponse.json(
      error("internal_error", "Failed to import employees", traceId),
      { status: 500 }
    );
  }
}
