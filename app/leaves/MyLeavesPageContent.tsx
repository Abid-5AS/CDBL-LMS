...
export function MyLeavesPageContent() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <section className="rounded-2xl border border-border/50 dark:border-slate-800/50 backdrop-blur-xl bg-white/70 dark:bg-card/70 p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-foreground mb-2">My Leave Requests</h1>
      ...
      <Card className="overflow-hidden">
        <CardHeader className="border-b border-border dark:border-slate-700 bg-accent/50 dark:bg-card/20 backdrop-blur-sm">
          <CardTitle>Requests</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <Table>
              <TableHeader className="sticky top-0 z-10 bg-card dark:bg-card border-b border-border dark:border-slate-700">
                <TableRow className="hover:bg-transparent">
                  <TableHead className="text-foreground dark:text-slate-200">Type</TableHead>
                  <TableHead className="hidden sm:table-cell text-foreground dark:text-slate-200">Dates</TableHead>
                  <TableHead className="hidden md:table-cell text-foreground dark:text-slate-200">Working Days</TableHead>
                  <TableHead className="text-foreground dark:text-slate-200">Status</TableHead>
                  <TableHead className="hidden lg:table-cell text-foreground dark:text-slate-200">Last Updated</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow>
                  <TableCell colSpan={6} className="text-center text-sm text-muted-foreground dark:text-slate-300 py-8">
                    Loading...
                  </TableCell>
                </TableRow>
              ) : error ? (
                ...
                filteredRows.map((row) => (
                  ...
                  >
                    <TableCell className="font-medium text-foreground">{leaveTypeLabel[row.type] ?? row.type}</TableCell>
                    <TableCell className="hidden sm:table-cell text-muted-foreground dark:text-slate-300">
                      {formatDate(row.startDate)} → {formatDate(row.endDate)}
                    </TableCell>
                    <TableCell className="hidden md:table-cell text-muted-foreground dark:text-slate-300">{row.workingDays}</TableCell>
                    <TableCell>
                      <StatusBadge status={row.status} />
                    </TableCell>
                    <TableCell className="hidden lg:table-cell text-muted-foreground dark:text-slate-300">{formatDate(row.updatedAt)}</TableCell>
                    <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                      {CANCELABLE_STATUSES.has(row.status) ? (
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant="outline" size="sm" className="border-slate-300 dark:border-slate-600 dark:bg-card dark:text-slate-200 dark:hover:bg-slate-700 dark:hover:border-slate-500">
                              Cancel
                            </Button>
                          </AlertDialogTrigger>
                          <AlertDialogContent>
